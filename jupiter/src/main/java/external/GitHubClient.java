...
public class GitHubClient {
	...
	private List<Item> getItemList(JSONArray array) {
		...
		String[] descArray = descriptionList.toArray(new String[descriptionList.size()]);
		List<List<String>> keywords = MonkeyLearnClient.extractKeywords(descArray);
		for (int j = 0; j < keywords.size(); j++) {
			List<String> list = keywords.get(j);
			Set<String> set = new HashSet<String>(list);
			itemList.get(j).setKeywords(set);
		}
